...
var ImgDiv= document.querySelectorAll('div[data-listings-container-wrapper]');
if (ImgDiv[0]!=null)
  ImgDiv=ImgDiv[0].querySelectorAll('a[data-palette-listing-image]');
else
  ImgDiv=null;
if (ImgDiv!=null){
...
async function StartDownload(mes,sender,sendRes){
  ...
  if (mes.from=="background"){
      ...
      ImgDiv=document.querySelectorAll('div[data-listings-container-wrapper]')[0].querySelectorAll('a[data-palette-listing-image]');
      for (img of ImgDiv){		
      ...
      if (mes.remain > 1){
        await sleep(200);
        var nextLI=document.querySelectorAll('div[data-item-pagination]');
        if (nextLI[0]==null) {
          chrome.runtime.sendMessage({
            from:"content",
            txt:"finished",
        ...
        var nextLink=nextLI[0].querySelectorAll('nav[aria-label]');
        if (nextLink[0]==null) {
          chrome.runtime.sendMessage({
            from:"content",
            txt:"finished",
